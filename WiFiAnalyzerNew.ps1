...
.DESCRIPTION
  Provides Guided and Quick start flows, parses `netsh wlan` output, recommends channels for 2.4/5/6 GHz,
...
function Get-WiFiScan {
  ...
  for ($i = 0; $i -lt $lines.Length; $i++) {
    ...
    if ($line -match "^BSSID\s+\d+\s*:\s*(.+)$") {
      $bssid = $matches[1].Trim(); $signal = $null; $channel = $null; $width = $null; $band = $null
      for ($j = 1; $j -le 10; $j++) {
        ...
        elseif ($next -match "^Band\s*:\s*(.+)$") { $band = $matches[1].Trim() }
      }
      if ($bssid -and $null -ne $signal -and $null -ne $channel) {
        if ($ssid) {
          $ssidDisplay = $ssid
        }
        else {
        ...
        elseif ($band -match '2[.,]4|2\.4') {
          $bandDisplay = '2.4 GHz'
        }
        elseif ($channel -le 14) {
          $bandDisplay = '2.4 GHz'
        }
        else {
          $bandDisplay = '5 GHz'
...
function Test-ChannelCongestion($networks) {
  # Include the band in each key because 2.4, 5, and 6 GHz can reuse channel numbers.
  $channelAnalysis = @{}
  foreach ($n in $networks) {
    ...
    if ($band -eq '2.4 GHz') {
      ...
      for ($i = $start; $i -le $end; $i++) {
        $dist = [Math]::Abs($i - $ch); $impact = $sig * (1 - ($dist / 5)); $key = "2.4 GHz|$i"
        if ($channelAnalysis.ContainsKey($key)) { $channelAnalysis[$key] += $impact } else { $channelAnalysis[$key] = $impact }
      }
    }
    else {
      $key = "$band|$ch"
      if ($channelAnalysis.ContainsKey($key)) { $channelAnalysis[$key] += $sig } else { $channelAnalysis[$key] = $sig }
    }
  }
...
function Get-BestChannel($networks) {
  if (-not $networks -or $networks.Count -eq 0) {
    return @{ '2.4GHz' = 'N/A'; '5GHz' = 'N/A'; '6GHz' = 'N/A'; CongestionData = @{} }
  }
  $cong = Test-ChannelCongestion $networks
  $result = @{ '2.4GHz' = 'N/A'; '5GHz' = 'N/A'; '6GHz' = 'N/A'; CongestionData = $cong }
  foreach ($bandInfo in @(
      @{ Label = '2.4 GHz'; Key = '2.4GHz' },
      @{ Label = '5 GHz'; Key = '5GHz' },
      @{ Label = '6 GHz'; Key = '6GHz' }
    )) {
    $candidate = $cong.GetEnumerator() | Where-Object { $_.Name -like "$($bandInfo.Label)|*" } | Sort-Object Value | Select-Object -First 1
    if ($candidate) { $result[$bandInfo.Key] = ($candidate.Name -split '\|', 2)[1] }
  }
  return $result
}
...
function Show-MainWiFiAnalyzerForm {
  ...
  $btnScan.Add_Click({
      ...
      $rec = Get-BestChannel $app.Networks; $app.Recommended = @{ '2.4GHz' = $rec.'2.4GHz'; '5GHz' = $rec.'5GHz'; '6GHz' = $rec.'6GHz' }; $app.CongestionData = $rec.CongestionData
      ...
      foreach ($band in @('2.4 GHz', '5 GHz', '6 GHz')) {
        $rtb.SelectionFont = New-Object Drawing.Font('Segoe UI', 10, [Drawing.FontStyle]::Bold)
        $rtb.AppendText("$band Band:`r`n")
        $rtb.SelectionFont = New-Object Drawing.Font('Segoe UI', 10)
        $bandChannels = $app.CongestionData.GetEnumerator() | Where-Object { $_.Name -like "$band|*" } | Sort-Object { [int](($_.Name -split '\\|', 2)[1]) }
        foreach ($entry in $bandChannels) {
          if ($entry.Value -gt 150) { $lvl = 'High' } elseif ($entry.Value -gt 75) { $lvl = 'Medium' } else { $lvl = 'Low' }
          $channelNumber = ($entry.Name -split '\\|', 2)[1]
          $rtb.AppendText("  Channel $channelNumber: $lvl congestion (Score: $([math]::Round($entry.Value,1)))`r`n")
        }
        if (-not $bandChannels) { $rtb.AppendText("  No $band networks detected.`r`n") }
        $rtb.AppendText("`r`n")
      }

      $rtb.SelectionFont = New-Object Drawing.Font('Segoe UI', 11, [Drawing.FontStyle]::Bold); $rtb.AppendText("Recommended Channels:`r`n"); $rtb.SelectionFont = New-Object Drawing.Font('Segoe UI', 10)
      $rtb.AppendText("2.4 GHz Band: Channel $($app.Recommended.'2.4GHz')`r`n")
      $rtb.AppendText("5 GHz Band  : Channel $($app.Recommended.'5GHz')`r`n")
      $rtb.AppendText("6 GHz Band  : Channel $($app.Recommended.'6GHz')`r`n`r`n")
...
function Show-ExportInfoForm {
  # (redeclared to ensure forward reference in single-file script)
