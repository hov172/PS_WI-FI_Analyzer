...
function Get-BestChannel($networks) {
    ...
    $congestion = Test-ChannelCongestion $networks
...
function Test-NetworkSpeed {
    ...
    try {
        ...
        $pingSuccess = $false
    # $successServer variable removed (was unused)
        $pingErrorCount = 0
...
function Show-MainWiFiAnalyzerForm {
    ...
    $scanButton.Add_Click({
        ...
        $recommendationData = Get-BestChannel $script:networks
        $script:recommended = @{
    ...
    $exportButton.Add_Click({
        if ($script:networks.Count -gt 0) {
            ...
            if ($userInfo) {
                Export-WiFiReport -networks $script:networks -mac $script:mac `
...
# All-in-one command: Invoke-WiFiAnalyzerAll
function Invoke-WiFiAnalyzerAll {
    param(
        [switch]$SkipReportInfo,
        [ValidateSet('HTML','CSV')][string]$OutputFormat = 'HTML',
        [string]$OutputPath
    )
    ...
    $extras = Get-ComputerInfoExtras
    $ssidInfo = Get-ConnectedSSID
    $recommended = Get-BestChannel $networks
    $speedTest = Test-NetworkSpeed
    $userInfo = $null
    if (-not $SkipReportInfo) {
        $userInfo = Show-ExportInfoForm
        if (-not $userInfo) {
            Write-Host "User info collection cancelled. Aborting report export." -ForegroundColor Red
            return
        }
    } else {
        $userInfo = @{ Name=''; ID=''; Email=''; Phone=''; Building=''; RoomNumber='' }
    }
    Export-WiFiReport -networks $networks -mac $mac -recommendedChannels $recommended `
        -computerName $extras.ComputerName -ipAddress $extras.IPAddress -userInfo $userInfo `
        -connectedSSID $ssidInfo.SSID -connectedBSSID $ssidInfo.BSSID -speedTest $speedTest -congestionData $recommended.CongestionData
    Write-Host "WiFi analysis and report export complete." -ForegroundColor Green
}